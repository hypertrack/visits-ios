...
let stateRestorationReducer: Reducer<AppState, AppAction, SystemEnvironment<AppEnvironment>> = Reducer { state, action, environment in
          ...
          case .none:
            return environment
              ...
              .flatMap { storageState -> Effect<AppAction, Never> in
                ...
                case let .some(.visits(visits, tabSelection, publishableKey, driverID)):
                  return environment
                    .hyperTrack
                    ...
                    .map { (result: (status: SDKStatus, permissions: Permissions)) in
                      ...
                      case let .unlocked(deviceID, unlockedStatus):
                        ...
                        return AppAction.restoredState(.left(.visits(visits, tabSelection, publishableKey, driverID, deviceID, unlockedStatus, result.permissions)), network)
                      }
                    }
                }
              }
  ...
  case let (.appLaunching, .restoredState(either, n)):
    ...
    case let .left(restoredState):
      ...
      case let .visits(visits, tabSelection, publishableKey, driverID, deviceID, unlockedStatus, permissions):
        
        state.flow = .visits(filterOutOldVisits(visits, now: environment.date()), nil, tabSelection, publishableKey, driverID, deviceID, unlockedStatus, permissions, nil, nil)
        return .merge(
          stateRestored,
          environment
            .hyperTrack
